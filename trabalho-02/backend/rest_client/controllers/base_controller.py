class BaseController:
    ...
    # TODO: see a better way to handle this later, maybe use python's built in http exceptions
    ...
    def _encrypt_response_body(self, response_body):
        return self.crypto_serializer.encrypt(response_body)
    ...
    def encrypt_response(self, response_body, client_id, keys_to_ignore):
        if client_id == 'web-socket-server':
            return self._encrypt_web_socket_response_body(response_body, keys_to_ignore)
        elif client_id == 'cmd-client':
            return self._encrypt_response_body(response_body)
        elif client_id == 'test-testudo':
